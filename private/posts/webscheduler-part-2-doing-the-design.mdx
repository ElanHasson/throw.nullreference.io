export const metadata = {
  ...
  featuredImage: "/posts/webscheduler-part-2-doing-the-design/PhysicalDeploymentDiagram.png",
  publishDate: "2022-09-13",
  lastModified: "2022-09-13",
  author: {
    name: "Elan Hasson",
    bio: "Software Developer and Tech Enthusiast",
    avatar: "/authors/avatar-1.jpg",
  },
  excerpt: "In this installment of building a distributed task scheduler, we'll go over the system design and implementation details using Microsoft Orleans and DigitalOcean App Platform.",
  tags: ["dotnet", "aspnet-core", "blazor", "digitalocean", "app-platform", "microsoft-orleans", "systems-design", "webscheduler"],
  categories: ["dotnet", "aspnet-core", "blazor", "digitalocean-app-platform", "microsoft-orleans", "systems-design", "webscheduler"],
  series: "Building a Distributed Task Scheduler on DigitalOcean App Platform",
  seo: {
    metaTitle: "WebScheduler Part II: Designing the Web Scheduler",
    metaDescription: "System design and implementation details for building a distributed task scheduler using Microsoft Orleans, ASP.NET Core, and DigitalOcean App Platform.",
  },
};

This is the second post in the series, if you haven't read the first, you should stop here and [read the first part](/blog/building-a-distributed-task-scheduler-on-digitalocean-app-platform/) before continuing. 
...
```
flowchart TB
    subgraph IdentityServer ["🌊 identity-server"]
        IdentityServerMySQL["🐬 identity-server-mysql"]
        IdentityServerHost["🐳 IdentityServer.Host"]
        IdentityServerHost --> IdentityServerMySQL
    end
    
    subgraph WebScheduler ["🌊 web-scheduler"]
    ...
    WebSchedulerFrontend --> IdentityServerHost
```
...
<Figure src="/posts/webscheduler-part-2-doing-the-design/orleans.png" width={100} alt="Microsoft Orleans Logo" className="float-left" />

[Microsoft Orleans](https://github.com/dotnet/orleans) is an open-source, cross-platform framework for building robust, scalable distributed applications in .NET. When I discovered Orleans sometime around 2015 I fell in love with it. It is the type of technology that one can get really excited about, the kind that changes mental model of problems and solutions. I've built a few solutions over the years with Orleans but have recently begun making real contributions (*non-docs* 😅) to the project.

If you're interested, join the [Official Orleans Discord chat](https://aka.ms/orleans-discord)™, it's by far the most welcoming project community I've ever been a part of!
...
* Reminders can only scheduled ≤49 days (`0xfffffffe` milliseconds) in the future
* Reminders can only tick at an interval of ≤49 days (`0xfffffffe` milliseconds)
...
<Figure src="/posts/webscheduler-part-2-doing-the-design/OrleansDashboard.png" width={440} alt="The OrleansDashboard" className="float-right" />
...
<Figure src="/posts/webscheduler-part-2-doing-the-design/Blazor.png" width={100} alt="ASP.NET Core Blazor Logo" className="float-right" />
...
```
flowchart LR
    subgraph BlazorApp ["WebScheduler.FrontEnd.BlazorApp"]
        Blazor["🔷 Blazor WebAssembly"]
    end
    
    subgraph WebAPI ["WebScheduler.Api"]
        APIHost["Web API Host"]
        APIHealth["Health Checks"]
        APIHealth --> APIHost
    end
    
    subgraph OrleansServer ["WebScheduler.Server"]
        Silo["🟣 Orleans Silo"]
        Dashboard["Orleans Dashboard"]
        OrleansHealth["Health Checks"]
        Dashboard --> Silo
        OrleansHealth --> Silo
    end
...
> **Info:** The code for the project can be found here [in these repositories](https://github.com/web-scheduler).
